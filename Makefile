...
build: $(RELEASE_BINARY) ## Build an optimized Stella bridge.
...
lint: ## Check Objective-C, shell, Markdown, and LaunchAgent files.
	...
	@temporary="$$(mktemp -d)"; \
		...
		caddy hash-password --plaintext stella-ci-validation > "$$temporary/password.hash"; \
		printf 'stella-ci %s\n' "$$(< "$$temporary/password.hash")" > "$$temporary/users.caddy"; \
		STELLA_API_HOST=stella.invalid \
...
install: build ## Install the CLI and read-only source/configuration assets.
	...
	install -m 0644 "$(SOURCE)" "$(INSTALL_DATADIR)/src/stella-bridge.m"
